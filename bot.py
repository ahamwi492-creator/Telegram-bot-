...
print("🚀 بدء تشغيل بوت الترندينغ الحقيقي - الإصدار المعدل V5.3 (إلغاء البيانات الاحتياطية)...")
...
class RealDEXTrendingBot:
    ...
    # ❌ تم حذف دالة البيانات الاحتياطية: نعتمد فقط على البيانات الحية
    ...
    def check_token_conditions(self, token):
        """فحص الشروط للتركيز على العملات النشطة في آخر 5 دقائق"""
        try:
    ...
    # 🌟 تم تشديد منطق الإشارة لزيادة نقاط الجودة
    def analyze_real_token(self, token):
        ...
        try:
            ...
            # 1. تقييم الجودة (السيولة والحجم) - نقاط أعلى للسيولة لضمان الجودة
            if liquidity > 500000: signal_score += 4  # سيولة عالية جداً
            ...
            if signal_score >= 10:
                ...
                signal = "⚠️ للمراجعة | بيانات ضعيفة"
                emoji = "⚠️"
                urgency = "🕒 عادي"
...
"""
    ...
    def run_real_analysis(self):
        ...
        # إذا فشل API الرئيسي، نتوقف (لا بيانات احتياطية)
        if not all_pairs:
            error_message = f"""
...
"""
        ...
        if not solana_pairs:
            print("⚠️ لم يتم العثور على عملات Solana، التوقف.")
            return 0
...
💎 **مصدر البيانات:** DEX Screener API مباشر (V5.3)
...
if __name__ == "__main__":
    ...
    try:
        start_message = f"""
🤖 **بدء بوت الترندينغ الحقيقي - الإصدار 5.3 (V5.3)**

⏰ {datetime.now().strftime('%Y-%m-%d %I:%M %p')}
🎯 **تحسينات الموثوقية:** تم إلغاء البيانات الاحتياطية لضمان استخدام بيانات حية فقط.
🔄 التكرار: كل 5 دقائق
"""
