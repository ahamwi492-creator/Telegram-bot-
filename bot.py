...
# 🚀 استراتيجية البحث لزيادة الموثوقية + جلب مباشر كخطة بديلة
SOLANA_DEX_ID = "solana"
SEARCH_API_URL = "https://api.dexscreener.com/latest/dex/search"
PAIR_API_URL = "https://api.dexscreener.com/latest/dex/pairs"

# Pair address لعملة PWINGA كمثال للاختبار والتحقق من الـ API المباشر
# يُفضل في الواقع عدم إضافة عناوين محددة، ولكن هذا للاختبار لضمان عمل API المباشر
PWINGA_PAIR_ADDRESS = "9S16V7H6sT282f6Qf8K9C1V2x7J1M8y8Q1Z1R7D9H0X9" 
# ملاحظة: تم استخدام عنوان زوج placeholder لأن العنوان الحقيقي غير متاح، يرجى استبداله بعنوان زوج نشط معروف.

print(f"🚀 بدء تشغيل بوت الترندينغ الحقيقي - الإصدار المعدل V6.2 (تنويع في الجلب و 25K سيولة)")
...
class RealDEXTrendingBot:
    ...
    def fetch_data_from_api(self, url, params=None):
        """دالة مساعدة لجلب البيانات من أي API لـ DEX Screener"""
        headers = {
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"❌ خطأ في API ({response.status_code}) من {url}.")
                return None
            data = response.json()
            pairs = data.get('pairs')
            if not isinstance(pairs, list):
                print(f"❌ تنسيق الأزواج غير صالح أو مفقود في {url}. Received type: {type(pairs)}.")
                return []
            return pairs
        except Exception as e:
            print(f"❌ خطأ غير متوقع في جلب البيانات من {url}: {e}")
            return []

    def get_multi_pool_data(self):
        """تنويع في مصادر الجلب بين البحث والمباشر"""
        
        # 1. 🔍 محاولة الجلب عبر البحث (الأكثر موثوقية حتى الآن)
        print("🔄 محاولة الجلب عبر API البحث ('raydium')...")
        search_pairs = self.fetch_data_from_api(SEARCH_API_URL, params={'q': 'raydium', 'limit': 50})
        
        all_pairs = search_pairs if isinstance(search_pairs, list) else []
        
        if not all_pairs:
            print("⚠️ نتائج البحث فارغة أو غير صالحة. محاولة خطة الطوارئ (الجلب المباشر).")
            # 2. ⚡️ خطة الطوارئ: الجلب المباشر (باستخدام عنوان زوج معروف)
            # هذه الخطوة للتحقق من أن API الجلب المباشر يعمل كخطة بديلة (V5.4 فشل هنا بخطأ 404)
            
            # ملاحظة: يجب تعديل عنوان الزوج هذا إلى عنوان زوج نشط على Solana إذا لم يعمل
            direct_url = f"{PAIR_API_URL}/{SOLANA_DEX_ID}/{PWINGA_PAIR_ADDRESS}"
            direct_pair_data = self.fetch_data_from_api(direct_url)
            
            if direct_pair_data and direct_pair_data[0]:
                print(f"✅ نجح الجلب المباشر لـ {PWINGA_PAIR_ADDRESS}")
                # نستخدم فقط الزوج الأول
                all_pairs = [direct_pair_data[0]] 
            else:
                print("❌ فشل الجلب المباشر أيضاً.")
                return []

        # ترتيب الأزواج حسب تاريخ الإنشاء (الأحدث أولاً)
        all_pairs.sort(key=lambda p: p.get('pairCreatedAt', 0), reverse=True)
        
        print(f"✅ تم جلب {len(all_pairs)} زوج للتحليل.")
        return all_pairs
    
    # ... (بقية الدوال filter_solana_tokens لم تتغير عن V6.1)
    ...
    # 🌟 شروط صارمة: (30 دقيقة، 5% 5M، و 25K$ سيولة)
    def check_token_conditions(self, token):
        """فحص الشروط للتركيز على العملات النشطة (كما في V6.1)"""
        try:
...
"""
    ...
    def run_real_analysis(self):
        ...
        all_pairs = self.get_multi_pool_data() 
...
⏰ {datetime.now().strftime('%I:%M %p')}
⚠️ توقف البوت: فشل جلب بيانات الترندينغ/البحث.
"""
...
💎 **مصدر البيانات:** DEX Screener API بحث/مباشر (V6.2)
...
if __name__ == "__main__":
    ...
    try:
        start_message = f"""
🤖 **بدء بوت الترندينغ الحقيقي - الإصدار 6.2 (V6.2)**

⏰ {datetime.now().strftime('%Y-%m-%d %I:%M %p')}
🎯 **تحسينات الجلب:** تم إضافة خطة طوارئ لاستخدام API الجلب المباشر (`/pairs`) لضمان موثوقية العملات سريعة الانتشار.
🔄 التكرار: كل 5 دقائق
"""
