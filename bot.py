...
# 🚀 تغيير نوع API لضمان جلب بيانات حية وصحيحة
SOLANA_DEX_ID = "solana"
REAL_TRENDING_API_URL = f"https://api.dexscreener.com/latest/dex/pairs/{SOLANA_DEX_ID}"

print(f"🚀 بدء تشغيل بوت الترندينغ الحقيقي - الإصدار المعدل V5.4 (استبدال API الترندينغ بـ: {SOLANA_DEX_ID})")
...
class RealDEXTrendingBot:
    ...
    def get_real_trending_data(self):
        """جلب بيانات أحدث الأزواج الحية من DEX Screener API (بديل للـ Trending API)"""
        try:
            print(f"🔄 جلب أحدث بيانات شبكة {SOLANA_DEX_ID} من DEX Screener API...")
            ...
            # 🎯 استخدام API أزواج DEXScreener
            response = requests.get(REAL_TRENDING_API_URL, headers=headers, timeout=20) 
            
            if response.status_code != 200:
                print(f"❌ خطأ في API ({response.status_code}): تعذر جلب البيانات. قد يكون API غير مستجيب.")
                return []
            
            data = response.json()
            
            # التحقق من أن المفتاح 'pairs' موجود ومحتواه قائمة
            pairs = data.get('pairs')
            
            if not isinstance(pairs, list):
                print(f"❌ تنسيق الأزواج غير صالح أو مفقود (Pairs): {type(pairs)}")
                return []
            
            print(f"✅ تم جلب البيانات بنجاح من API. عدد الأزواج المستلمة: {len(pairs)}")
            
            # ترتيب الأزواج المجلوبة حسب تاريخ الإنشاء (الأحدث أولاً) 
            # هذا ضروري لأن API pairs/{chainId} يجلب جميع الأزواج، لكننا نريد الأحدث في البداية.
            pairs.sort(key=lambda p: p.get('pairCreatedAt', 0), reverse=True)
            
            # التركيز على أول 50 زوج فقط (لتقليل الحمل)
            return pairs[:50]
    ...
    # ❌ تم حذف دالة filter_solana_tokens (لأننا نطلب Solana مباشرة)

    # 🌟 شروط صارمة: (30 دقيقة، 5% 5M، و 50K$ سيولة)
    def check_token_conditions(self, token):
        ...
        try:
            ...
            # 5. تحقق إضافي من وجود العناوين
            if not token.get('pairAddress') or not base_token.get('address'):
                print("   ❌ عنوان الزوج أو التوكن مفقود. تم الرفض لضمان الجودة.")
                return False
    ...
    # 🌟 تم تشديد منطق الإشارة لزيادة نقاط الجودة (كما في V5.3)
    def analyze_real_token(self, token):
        ...
        try:
            ...
            if signal_score >= 10:
                ...
                # إذا كانت النقاط منخفضة جداً، نعتبرها غير مؤهلة حتى لو مرت بالفلتر (احتياط)
                return None 
...
"""
    ...
    def run_real_analysis(self):
        ...
        # جلب البيانات الحقيقية (أحدث أزواج Solana)
        solana_pairs = self.get_real_trending_data()
        
        # لا توجد دالة filter_solana_tokens، لأننا نطلب Solana مباشرة
        
        if not solana_pairs:
            error_message = f"""
❌ **تعذر جلب بيانات Solana الحية**

⏰ {datetime.now().strftime('%I:%M %p')}
⚠️ توقف البوت: لا يمكن الاعتماد على أي مصدر حالياً.
"""
        ...
        if analyzed_tokens:
            # الترتيب حسب الزخم (change_5m) والسيولة (liquidity)
...
💎 **مصدر البيانات:** DEX Screener API مباشر (V5.4)
...
if __name__ == "__main__":
    ...
    try:
        start_message = f"""
🤖 **بدء بوت الترندينغ الحقيقي - الإصدار 5.4 (V5.4)**

⏰ {datetime.now().strftime('%Y-%m-%d %I:%M %p')}
🎯 **تحسينات الموثوقية:** تم تغيير مصدر البيانات للتعامل مع أخطاء الـ API وضمان بيانات حية وأحدث.
🔄 التكرار: كل 5 دقائق
"""
