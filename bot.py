...
print("🚀 بدء تشغيل بوت الترندينغ الحقيقي - الإصدار المعدل V5.1 (عمر 30 دقيقة)...")
...
class RealDEXTrendingBot:
        ...
        try:
            ...
            for i, pair in enumerate(pairs[:3]):
                if pair and isinstance(pair, dict):
                    ...
                    # لا يلزم طباعة كل شيء هنا، فقط للعرض
    ...
    # 🌟 التعديل الأساسي هنا: تشديد الشروط على 30 دقيقة
    def check_token_conditions(self, token):
        ...
        try:
            ...
            # رفع الحد الأدنى للسيولة إلى 50,000$ (مهم جداً)
            MIN_LIQUIDITY = 50000 
            ...
            # 2. فحص الأداء في 5 دقائق - هذا هو الشرط الأهم للـ 5M trending
            ...
            # 🌟 الشرط الأساسي لتقليد Trending 5M: يجب أن يكون الارتفاع في آخر 5 دقائق كبيراً
            MIN_5M_CHANGE = 10.0 # حد أدنى 10% ارتفاع في 5 دقائق
            if change_5m < MIN_5M_CHANGE:
                print(f"   ❌ ارتفاع 5 دقائق ضعيف (أقل من {MIN_5M_CHANGE}%)")
                return False
            
            # 3. فحص العمر الحقيقي - تم تعديل هذا الشرط
            ...
            MIN_VOLUME_24H = 50000 # 50 ألف دولار كحد أدنى (تم تخفيضه قليلاً للتركيز على الزخم)
    ...
    # لم يتم تعديل الدالة (analyze_real_token) لكنها تستخدم الشروط الصارمة الآن
...
🎯 **بوت الترندينغ الحقيقي v5.1 (المعدل)**
"""
            return message
...
✅ تم فحص {len(solana_pairs)} عملة، ولكن لم تستوفِ أي منها شروط الجودة الصارمة (30 دقيقة).
⏰ {datetime.now().strftime('%I:%M %p')}
...
🎯 **التركيز:** عملات بعمر لا يتجاوز 30 دقيقة.
...
if __name__ == "__main__":
    ...
    try:
        # رسالة البدء
        start_message = f"""
🤖 **بدء بوت الترندينغ الحقيقي - الإصدار 5.1 (30 دقيقة)**

⏰ {datetime.now().strftime('%Y-%m-%d %I:%M %p')}
🎯 **التحسينات:** التركيز على العملات الجديدة جدًا (أقل من 30 دقيقة) مع زخم في 5 دقائق.
🔄 التكرار: كل 5 دقائق
