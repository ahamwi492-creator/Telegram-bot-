...
# 🚀 العودة إلى API الترندينغ العام مع معالجة محسنة للأخطاء
REAL_TRENDING_API_URL = "https://api.dexscreener.com/latest/dex/tokens/trending"

print(f"🚀 بدء تشغيل بوت الترندينغ الحقيقي - الإصدار المعدل V5.5 (العودة لـ API Trending مع معالجة Nulls)")
...
class RealDEXTrendingBot:
    ...
    def get_real_trending_data(self):
        """جلب بيانات الترندينغ من API الأصلي مع معالجة مشكلة NoneType"""
        try:
            print("🔄 جلب البيانات الحية من DEX Screener API (Trending)...")
            ...
            if response.status_code != 200:
                print(f"❌ خطأ في API ({response.status_code}): تعذر جلب البيانات.")
                return []
            
            data = response.json()
            
            # 🎯 معالجة مشكلة NoneType هنا: التأكد من أن المفتاح 'pairs' موجود وقائمة
    ...
    def filter_solana_tokens(self, pairs):
        """تصفية عملات Solana فقط والتحقق من البيانات الأساسية (V5.5)"""
        if not pairs:
            return []
        
        solana_pairs = []
        for pair in pairs:
            # التحقق من أن العملة على شبكة Solana
            if pair and isinstance(pair, dict) and pair.get('chainId') == 'solana':
                
                # 🎯 فحص البيانات الأساسية ضد العملات الميتة أو غير الموجودة
                ...
                if liquidity_usd > 0 and price_usd is not None and pair_address:
                    ...
                    print(f"   ⚠️ تجاهل عملة بدون سيولة/سعر/عنوان زوج: {pair.get('baseToken', {}).get('symbol')}")
    ...
    def check_token_conditions(self, token):
        ...
        try:
            # تأكدنا من البيانات الأساسية في filter_solana_tokens، الآن نطبق الشروط
...
"""
    ...
    def run_real_analysis(self):
        ...
        if not all_pairs:
            error_message = f"""
❌ **تعذر جلب البيانات الحية من API**

⏰ {datetime.now().strftime('%I:%M %p')}
⚠️ توقف البوت: فشل جلب بيانات الترندينغ.
"""
        ...
        if not solana_pairs:
            print("⚠️ لم يتم العثور على عملات Solana مؤهلة بعد التحقق من السيولة/السعر/العقد.")
            return 0
...
💎 **مصدر البيانات:** DEX Screener API مباشر (V5.5)
...
if __name__ == "__main__":
    ...
    try:
        start_message = f"""
🤖 **بدء بوت الترندينغ الحقيقي - الإصدار 5.5 (V5.5)**

⏰ {datetime.now().strftime('%Y-%m-%d %I:%M %p')}
🎯 **تحسينات الموثوقية:** تم العودة لـ API الترندينغ مع إضافة فحص صارم للسيولة/العقد/السعر لتجنب البيانات المضللة.
🔄 التكرار: كل 5 دقائق
"""
