...
# 🚀 API الترندينغ العام مع معالجة محسّنة للأخطاء
REAL_TRENDING_API_URL = "https://api.dexscreener.com/latest/dex/tokens/trending"

print(f"🚀 بدء تشغيل بوت الترندينغ الحقيقي - الإصدار المعدل V5.6 (معالجة NoneType مباشرةً في API Trending)")
...
class RealDEXTrendingBot:
    ...
    def get_real_trending_data(self):
        """جلب بيانات الترندينغ من API الأصلي مع معالجة مشكلة NoneType في المصدر"""
        try:
            ...
            # 🎯 معالجة NoneType: التحقق الصارم من أن 'pairs' موجودة وقائمة
            pairs = data.get('pairs')
            
            if not isinstance(pairs, list) or not pairs:
                print(f"❌ تنسيق الأزواج غير صالح أو مفقود. (Received type: {type(pairs)}). قد يكون API Trending فارغًا حاليًا.")
                return []
    ...
    def filter_solana_tokens(self, pairs):
        ...
        for pair in pairs:
            ...
            if pair and isinstance(pair, dict) and pair.get('chainId') == 'solana':
                ...
                # التحقق من أن السيولة موجودة ورقم صالح
...
"""
    ...
    def run_real_analysis(self):
        ...
        if not all_pairs:
            # إذا فشل الجلب بسبب NoneType أو خطأ آخر في API، سنرسل رسالة توقف
            error_message = f"""
❌ **تعذر جلب بيانات الترندينغ الحية من API**

⏰ {datetime.now().strftime('%I:%M %p')}
⚠️ توقف البوت: فشل جلب بيانات الترندينغ (Check API Health).
"""
...
💎 **مصدر البيانات:** DEX Screener API مباشر (V5.6)
...
if __name__ == "__main__":
    ...
    try:
        start_message = f"""
🤖 **بدء بوت الترندينغ الحقيقي - الإصدار 5.6 (V5.6)**

⏰ {datetime.now().strftime('%Y-%m-%d %I:%M %p')}
🎯 **تحسينات الموثوقية:** تم تشديد معالجة خطأ (NoneType) في مصدر البيانات لضمان موثوقية الجلب.
🔄 التكرار: كل 5 دقائق
"""
