...
# 🚀 استراتيجية البحث لزيادة الموثوقية (كما في V6.0)
SOLANA_DEX_ID = "solana"
SEARCH_API_URL = "https://api.dexscreener.com/latest/dex/search"

print(f"🚀 بدء تشغيل بوت الترندينغ الحقيقي - الإصدار المعدل V6.1 (سيولة 25K)")
...
class RealDEXTrendingBot:
    ...
    def get_multi_pool_data(self):
        """جلب البيانات من خلال البحث عن أحدث وأكثر الأزواج نشاطاً في Raydium (V6.0 Strategy)"""
        try:
            query = "raydium"  
            params = {
                'q': query,
    ...
    def check_token_conditions(self, token):
        """فحص الشروط للتركيز على العملات النشطة (تم تخفيف شرط السيولة)"""
        try:
            ...
            # 🎯 V6.1 تعديل: تخفيف السيولة إلى 25K
            MIN_LIQUIDITY = 25000 
            ...
            # ملاحظة: حجم 24h يفضل أن يبقى 50K للحفاظ على الجودة، يمكن تخفيفه لاحقاً إذا لزم الأمر.
    ...
    def analyze_real_token(self, token):
        ...
        try:
            ...
            # 1. تقييم الجودة (السيولة والحجم) (تم تعديل النقاط لتناسب السيولة الجديدة 25K)
...
💎 **مصدر البيانات:** DEX Screener API بحث (V6.1)
...
if __name__ == "__main__":
    ...
    try:
        start_message = f"""
🤖 **بدء بوت الترندينغ الحقيقي - الإصدار 6.1 (V6.1)**

⏰ {datetime.now().strftime('%Y-%m-%d %I:%M %p')}
🎯 **تحسينات الشروط:** تم تخفيف الحد الأدنى لـ **السيولة (Liquidity) إلى $25,000** بدلاً من $50,000 لزيادة المرونة في التقاط العملات السريعة.
🔄 التكرار: كل 5 دقائق
"""
