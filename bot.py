...
# 🚀 استبدال API الترندينغ العام بـ API البحث العام لزيادة الموثوقية
SOLANA_DEX_ID = "solana"
RAYDIUM_WETH_PAIR = "Bf8cERk5WJ9fH6Yy4M6cK1Yy3JbW7p8h3q9uFq5z8XqQ" # مثال لـ WETH/USDC على Raydium (قد يختلف)
# سنقوم بالبحث عن Pools شائعة بدلاً من عقد واحد غير موثوق به
SEARCH_API_URL = "https://api.dexscreener.com/latest/dex/search"

print(f"🚀 بدء تشغيل بوت الترندينغ الحقيقي - الإصدار V6.0 (استراتيجية البحث المتعدد)")
...
class RealDEXTrendingBot:
    ...
    def get_multi_pool_data(self):
        """جلب البيانات من خلال البحث عن أحدث وأكثر الأزواج نشاطاً في Raydium"""
        try:
            # 🎯 استهداف Raydium (المصدر الرئيسي لعملات Solana الجديدة)
            query = "raydium"  
            params = {
                'q': query,
                'limit': 50 # جلب أكبر عدد من النتائج لتحليلها
            }
            
            print(f"🔄 جلب بيانات البحث من DEX Screener API للـ Pools النشطة ({query})...")
            ...
            if response.status_code != 200:
                print(f"❌ خطأ في API البحث ({response.status_code}): تعذر جلب البيانات.")
                return []
            
            data = response.json()
            
            # التحقق من أن 'pairs' موجودة وقائمة
            pairs = data.get('pairs')
            
            if not isinstance(pairs, list) or not pairs:
                print(f"❌ تنسيق الأزواج غير صالح أو مفقود. (Received type: {type(pairs)}).")
                return []
            
            print(f"✅ تم جلب البيانات بنجاح من API البحث. عدد الأزواج المستلمة: {len(pairs)}")
            
            # ترتيب الأزواج حسب تاريخ الإنشاء (الأحدث أولاً) لتحديد الترندينغ
    ...
    # دالة تصفية Solana والتحقق من البيانات الأساسية (تم تعديلها لتناسب البحث)
    def filter_solana_tokens(self, pairs):
        ...
        solana_pairs = []
        for pair in pairs:
            # 1. التحقق من أن العملة على شبكة Solana
            if pair and isinstance(pair, dict) and pair.get('chainId') == SOLANA_DEX_ID:
                
                # 2. فحص البيانات الأساسية ضد العملات الميتة أو غير الموجودة
                ...
                # يجب أن تكون السيولة أكبر من الصفر، والسعر والعنوان موجودان.
    ...
    # 🌟 شروط صارمة (كما هي لضمان الجودة)
    def check_token_conditions(self, token):
        ...
        try:
            ...
            # الحد الأدنى لارتفاع 5 دقائق 5.0%
            MIN_5M_CHANGE = 5.0 
    ...
    def analyze_real_token(self, token):
        ...
        try:
            ...
            # **منطق الإشارة: يعتمد على نقاط الجودة والزخم**
            signal_score = 0
            
            # 1. تقييم الجودة (السيولة والحجم)
            if liquidity > 500000: signal_score += 4  
            elif liquidity > 200000: signal_score += 2
    ...
    # ... (بقية الدوال format_real_token_message, send_telegram_message لم تتغير)
...
"""
    ...
    def run_real_analysis(self):
        ...
        # جلب البيانات باستخدام استراتيجية البحث المتعدد
...
"""
        ...
        # تصفية عملات Solana (مع التحقق من السيولة/السعر/العقد)
...
💎 **مصدر البيانات:** DEX Screener API بحث (V6.0)
...
if __name__ == "__main__":
    ...
    try:
        start_message = f"""
🤖 **بدء بوت الترندينغ الحقيقي - الإصدار 6.0 (V6.0)**

⏰ {datetime.now().strftime('%Y-%m-%d %I:%M %p')}
🎯 **تحسينات الموثوقية الجذرية:** تم تبديل مصدر البيانات إلى API البحث (Search API) لاستقرار أعلى بدلاً من API الترندينغ غير الموثوق به (Trending API).
🔄 التكرار: كل 5 دقائق
"""
