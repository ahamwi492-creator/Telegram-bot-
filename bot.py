...
TRENDING_API_URL = "https://api.dexscreener.com/latest/dex/search/trending" # قد يعود بـ NoneType
...
class RealDEXTrendingBot:
    ...
    def fetch_data_from_api(self, url, params=None, source_name="API"):
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"❌ خطأ في {source_name} ({response.status_code}): تعذر جلب البيانات.")
                return None
            ...
            if not isinstance(pairs, list):
                print(f"❌ تنسيق الأزواج غير صالح أو مفقود في {source_name}: {type(pairs)}.")
                # نُعيد قائمة فارغة هنا للسماح بالمحاولة التالية بدلاً من التعطل
                return [] 
            ...
            print(f"❌ خطأ غير متوقع في جلب البيانات من {source_name}: {e}")
            return None

    def get_multi_source_data(self):
        """تنويع مصادر الجلب بين البحث (أساسي) والترندينغ (احتياطي)"""
        
        # 1. 🔍 المحاولة الأولى: API البحث (الأكثر استقراراً في التجارب الأخيرة)
        search_pairs = self.fetch_data_from_api(
            SEARCH_API_URL, 
            params={'q': 'raydium', 'limit': 75}, # زيادة الحد قليلاً
            source_name="API البحث"
        )
        
        if search_pairs is not None and isinstance(search_pairs, list) and search_pairs:
            # ترتيب الأزواج حسب تاريخ الإنشاء لتحديد الترند
            search_pairs.sort(key=lambda p: p.get('pairCreatedAt', 0), reverse=True)
            return search_pairs

        # 2. ⚡️ المحاولة الثانية: API الترندينغ (احتياطي، لكن معرض للـ NoneType)
        print("⚠️ فشل API البحث أو لم يعثر على بيانات. الانتقال إلى API الترندينغ الاحتياطي.")
    ...
    # 🌟 شروط صارمة (25K سيولة، 5% 5M، 30 دقيقة عمر) - لم تتغير
    def check_token_conditions(self, token):
        ...
        try:
            ...
            if not isinstance(price_change, dict): price_change = {}
...
🎯 **تحسينات الموثوقية الجذرية 3:** تم تفعيل استراتيجية **التنويع العكسي**، حيث يحاول البوت الجلب من **API البحث (`/search`)** أولاً، وإذا فشل، ينتقل إلى **API الترندينغ (`/trending`)** لضمان الحصول على بيانات صالحة من أي مصدر ممكن.
🔄 التكرار: كل 5 دقائق
"""
